...
import chapter08_tree.LinkedBinaryTree;
import chapter10.maps.AbstractSortedMap;
...
public class TreeMap<K, V> extends AbstractSortedMap<K, V> {
	...
	private int getHeight(Position<Entry<K, V>> node) {
		if(isExternal(node)) {
			return 0;
		}
		return 1 + Math.max(getHeight(right(node)), getHeight(left(node)));
	}
    ...
    public V put(K key, V value) throws IllegalArgumentException {
    	checkKey(key);							// may throw IllegalArgumentException
    	Entry<K, V> newEntry = (Entry<K, V>) new MapEntry(key, value);
    	Position<Entry<K, V>> p = treeSearch(root(), key);
